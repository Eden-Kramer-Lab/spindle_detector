...
def simulate_LFP(time, spindle_times, spindle_amplitude=2.0,
                 spindle_width=0.500, noise_type='brown', noise_amplitude=1.3,
                 spindle_frequency=13):
    '''Simulate a LFP with a spindle at spindle times
    '''
    noise = (noise_amplitude / 2) * NOISE_FUNCTION[noise_type](time.size)
    spindle_signal = np.sin(2 * np.pi * time * spindle_frequency)
    signal = []

    try:
        iter(spindle_times)
    except TypeError:
        spindle_times = [spindle_times]

    for spindle_time in spindle_times:
        carrier = norm(loc=spindle_time + (spindle_width / 2),
                       scale=spindle_width / 4).pdf(time)
        carrier /= carrier.max()
        signal.append((spindle_amplitude / 2) * (spindle_signal * carrier))
